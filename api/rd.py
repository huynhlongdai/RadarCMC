...
# Một hàm duy nhất, tự chứa, chỉ dùng thư viện chuẩn. vercel.json ánh xạ
# /api/health · /api/search · /api/scan vào ?op=... của hàm này. Trên Vercel
# phần proxy chạy phía máy chủ nên vấn đề CORS biến mất, và key CMC đặt ở
# biến môi trường CMC_API_KEY thì người xem không cần key riêng.
class handler(BaseHTTPRequestHandler):
    server_version = 'ExitRadar/2.0'

    def log_message(self, *a):
        pass

    def _out(self, code, payload):
    ...
    def do_GET(self):
        ...
        g = lambda k, d='': (q.get(k, [d])[0] or d)
        op = g('op') or 'scan'
        key = self.headers.get('X-CMC-Key') or os.environ.get('CMC_API_KEY') or None
        try:
            if op == 'health':
                probe = cmc('/v1/dex/search', {'q': 'BTC'}, key)
                return self._out(200, {'ok': True, 'server': 'exit-radar-vercel',
                                       'cmcReachable': probe['ok'], 'cmcStatus': probe['status'],
                                       'keyProvided': bool(key), 'runtime': 'vercel',
                                       'swapPages': SWAP_PAGES, 'stats': dict(STATS)})
            if op == 'search':
                return self._out(200, search(g('q'), key))
            if op == 'scan':
                pl, ad = g('platform'), g('address')
                if not pl or not ad:
                    return self._out(400, {'error': 'thiếu platform hoặc address'})
                out = build(pl, ad, key)
                return self._out(200 if 'error' not in out else 502, out)
            return self._out(404, {'error': 'không có op này'})
        except Exception as e:
            return self._out(500, {'error': str(e)})
